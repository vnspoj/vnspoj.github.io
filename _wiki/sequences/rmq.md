...
- [Segment tree](../data_structures/segment_tree) - answers each query in $O(\log N)$, preprocessing done in $O(N)$.
...
- [Disjoint Set Union / Arpa's Trick](../data_structures/disjoint_set_union#arpa) - answers queries in $O(1)$, preprocessing in $O(n)$. Pros: short, fast. Cons: only works if all queries are known in advance, i.e. only supports off-line processing of the queries.
- [Cartesian Tree](../graph/rmq_linear) and [Farach-Colton and Bender algorithm](../graph/lca_farachcoltonbender) - answers queries in $O(1)$, preprocessing in $O(n)$. Pros: optimal complexity. Cons: large amount of code.
